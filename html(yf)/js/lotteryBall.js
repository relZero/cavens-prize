var ballAnimate, ballTurn, ballBoom;

function ThreeBallInit(configObj) {
    var defaultConfig = {
        ballFlag: true,
        threeBallHead: "",
        ballBoxId: "container",
...
ThreeBallInit.prototype = {
    ...
    animate: function () {
        cancelAnimationFrame(ballTurn);
        cancelAnimationFrame(ballBoom);
        // 让场景通过x轴或者y轴旋转  & z
        // scene.rotation.x += 0.011;
        this.scene.rotation.y += 0.008;
        ballAnimate = requestAnimationFrame(this.animate.bind(this));
        TWEEN.update();
    ...
    turnAnimate: function () {
        cancelAnimationFrame(ballAnimate);
        cancelAnimationFrame(ballBoom);
        this.scene.rotation.y += 1;
        ballTurn = requestAnimationFrame(this.turnAnimate.bind(this));
        TWEEN.update();
        this.controls.update();
        // 渲染循环
        this.render();
    ...
    boomAnimate: function () {
        ...
        ballBoom = requestAnimationFrame(this.boomAnimate.bind(this));
