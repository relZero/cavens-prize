...
function getImage(canvas, ctx) {
  ...
  image.onload = function () {
      ...
      var indexWordHead = [];
      $.ajax({
        url: "http://localhost:3003/personnelData",
        type: "get",
        success: function(res){
          for (var i = 0; i < res.length; i++) {
              var headRes = res[i].staffHead;
              indexWordHead.push(headRes)
          }
          var indexWordInit = new IndexWordInit({
              ballBoxId: "indexContainer",
              threeBallHead: indexWordHead,
              picNumber: imgFix.length,
              picNumberCompared: indexWordHead.length,
              actionArr: imgFix
          });
        }
      });
