$(function () {
    ...
    $winnerEach.each(function () {
        ...
        excelToJsonInit.GetData("http://localhost:3004/" + thisPrize, "", function (data) {
    ...
    function deleteDataAction($this, thisTabPrize) {
        ...
        excelToJsonInit.GetData("http://localhost:3004/" + thisTabPrize + "?_sort=staffSort", {id: thisId}, function (data) {
            var staffData = data[0];
            excelToJsonInit.DeleteData("http://localhost:3004/" + thisTabPrize + "/", staffData.id, function () {
                $this.remove();
            });
    ...
    function backDataAction($this, thisTabPrize) {
        ...
        if (thisTabPrize === "bonus") {
            prizeUrl = "http://localhost:3006/bonusData";
        } else if (thisTabPrize === "table") {
            prizeUrl = "http://localhost:3007/tableData";
        } else {
            prizeUrl = "http://localhost:3003/personnelData";
        }
        excelToJsonInit.GetData("http://localhost:3004/" + thisTabPrize + "?_sort=staffSort", {id: thisId}, function (data) {
            var staffData = data[0];
            var sortNum;
